...
const getMyContributions = async (req, res) => {
    try {
       ...
       ORDER BY c.created_at DESC`, [donorId]);
        ...
        return (0, response_1.sendSuccess)(res, formattedContributions, 'Contributions fetched successfully');
    }
    catch (error) {
...
const getNgoContributions = async (req, res) => {
    try {
       ...
       INNER JOIN donations dr ON c.donation_id = dr.id
       WHERE dr.ngo_id = ?
       ORDER BY c.created_at DESC`, [requestedNgoId]);
        const formattedContributions = contributions.map((c) => ({
            id: c.id,
            donationId: c.donation_id,
            donorId: c.donor_id,
        ...
        return (0, response_1.sendSuccess)(res, formattedContributions, 'NGO contributions fetched successfully');
    }
    catch (error) {
