...
const authenticate = async (req, res, next) => {
    try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/ce196b78-abae-4f3c-8dd1-512bb7a3206c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'auth.middleware.ts:21', message: 'AUTH_NO_HEADER', data: { hasHeader: !!authHeader, startsWithBearer: authHeader === null || authHeader === void 0 ? void 0 : authHeader.startsWith('Bearer ') }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
            // #endregion
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        }
        const token = authHeader.split(' ')[1];
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/ce196b78-abae-4f3c-8dd1-512bb7a3206c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'auth.middleware.ts:25', message: 'AUTH_TOKEN_EXTRACTED', data: { tokenLength: token === null || token === void 0 ? void 0 : token.length, tokenPrefix: token === null || token === void 0 ? void 0 : token.substring(0, 20) }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'A' }) }).catch(() => { });
        // #endregion
        ...
        // IMPORTANT: Use token role to determine which table to check first
        const tokenRole = (payload.role || '').toUpperCase();
        const user = await (0, mysql_auth_helper_1.findUserById)(payload.userId, tokenRole);
        console.log(`[Auth Middleware] 🔍 User lookup result - Found: ${!!user}, Role from DB: "${user === null || user === void 0 ? void 0 : user.role}", Token Role: "${tokenRole}", ID: ${user === null || user === void 0 ? void 0 : user.id}`);
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/ce196b78-abae-4f3c-8dd1-512bb7a3206c', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ location: 'auth.middleware.ts:32', message: 'AUTH_USER_LOOKUP', data: { userId: payload.userId, userFound: !!user, userRole: user === null || user === void 0 ? void 0 : user.role, userIsBlocked: user === null || user === void 0 ? void 0 : user.isBlocked }, timestamp: Date.now(), sessionId: 'debug-session', runId: 'run1', hypothesisId: 'C' }) }).catch(() => { });
        // #endregion
        if (!user) {
            // #region agent log
