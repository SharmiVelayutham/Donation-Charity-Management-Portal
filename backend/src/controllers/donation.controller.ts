...
import { DonationModel, DonationType } from '../models/Donation.model';
...
const VALID_DONATION_TYPES: DonationType[] = ['FOOD', 'FUNDS', 'CLOTHES', 'MEDICINE', 'BOOKS', 'TOYS', 'OTHER'];
...
export const createDonation = async (req: AuthRequest, res: Response) => {
  ...
  return sendSuccess(res, populated, 'Donation created', 201);
};
...
export const getDonations = async (req: Request, res: Response) => {
  ...
  if (donationType) filter.donationType = { $regex: donationType as string, $options: 'i' };
  ...
  return sendSuccess(res, donationsWithCounts, 'Donations fetched');
};
...
export const getDonationById = async (req: Request, res: Response) => {
  ...
  return sendSuccess(res, donationWithCounts, 'Donation fetched');
};
...
export const updateDonation = async (req: AuthRequest, res: Response) => {
  ...
  if (location) {
    try {
      updates.location = normalizeLocation(location);
    } catch (error: any) {
      return res.status(400).json({ success: false, message: error.message || 'Invalid location format' });
    }
  }
...
export const cancelDonation = async (req: AuthRequest, res: Response) => {
  ...
  const updated = await DonationModel.findById(id)
    .populate('ngoId', 'name email contactInfo role');
...
export const getMyDonations = async (req: AuthRequest, res: Response) => {
  ...
  const filter: Record<string, unknown> = { ngoId: req.user!.id };
  ...
  return sendSuccess(res, donationsWithCounts, 'My donations fetched');
};
  ...
  const nearbyDonations = donations
    .map((donation) => {
      ...
      const distance = calculateDistance(
        ...
        donation.location.coordinates.longitude
      );
